...
/*   Updated: 2021/07/13 11:32:53 by cboutier         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	exec1(t_cmds *cmds, int *pfd, char **envp)
{
	...
	close(pfd[1]);
	if (execve(cmds->cmd1_w_path, cmds->cmd1, envp) == -1)
	{
...
void	exec2(t_cmds *cmds, int *pfd, char **envp)
{
	...
	if (fd_out == -1)
	{
		ft_putstr_fd("file2 open failed\n", 2);
		exit(1);
	}
	...
	close(pfd[0]);
	if (execve(cmds->cmd2_w_path, cmds->cmd2, envp) == -1)
	{
